...
40. **Android phone book contact name resolution** *(Commit `5054ac9`)* -- Created `PhoneBookResolver.kt` utility using `ContactsContract.PhoneLookup.CONTENT_FILTER_URI` as the PRIMARY source for contact names. Two methods: `resolvePhoneNumber()` (phone book only) and `resolveCallerName()` (priority chain: phone book → SecureCall contacts → raw phone → raw clientId). Integrated in 5 files: `IncomingCallActivity.kt` (caller name on ring screen), `WebSocketService.kt` (notification name), `CallActivity.java` (re-resolve raw IDs/numbers), `CallsFragment.kt` (enrich call history entries), `IncomingCallActivity.kt saveMissedCallFromIntent()`. Tested: S10 shows "S7 Handy" and "Σαννυ" from phone book; S7 shows "CHEF" from phone book.
41. **WebSocket reconnect rewrite with connection status** *(Commit `a58b4a0`)* -- `HeartbeatClient.kt`: Complete rewrite — owns ALL reconnection logic. Dead connection detection: 15s HEARTBEAT + 20s staleness check. `connect()` cancels old socket first. `onClosing()` now schedules reconnect. `forceReconnect()` cancels socket and schedules reconnect. Exponential backoff (1s, 2s, 4s, 8s, 16s, 30s max). `isClosed` flag prevents reconnect after intentional `close()`. `WebSocketService.kt`: Removed duplicate reconnect logic (`scheduleReconnect()`, `connectWebSocket()`, `reconnectAttempts`, `backoffSequenceMs`). `onDisconnected()` no longer reconnects (HeartbeatClient owns it). `handleHeartbeatTimeout()` delegates to `forceReconnect()`. Added `ensureConnected()` for pre-call verification. `MainActivity.java`: Connection status shows green dot (●) + "Connected" or red dot (●) + "Disconnected" in toolbar subtitle. `CallActivity.java`: Pre-call check — if WebSocket disconnected, triggers reconnect and shows "Reconnecting…".
...
- **Single reconnect owner:** HeartbeatClient owns ALL reconnection logic. `connect()` cancels old sockets, `onClosing()` and `onFailure()` both schedule reconnect, `forceReconnect()` for forced teardown. WebSocketService only delegates via `forceReconnect()` or `handleHeartbeatTimeout()` — it does NOT schedule reconnects itself. Dead connection detection: 15s HEARTBEAT with 20s staleness check. Exponential backoff: 1s → 30s max.
- **Contact name resolution priority:** `PhoneBookResolver.resolveCallerName()` checks (1) Android phone book via `ContactsContract.PhoneLookup` (primary source), (2) SecureCall internal contacts by clientId, (3) SecureCall contacts by phone, (4) raw phone number, (5) raw clientId. Used in IncomingCallActivity, CallActivity, WebSocketService notifications, and CallsFragment history enrichment.
...
│   │       │   ├── data/                       # ContactRepository, CallHistoryRepository, PhoneBookResolver (NEW: Android phone book lookup)
...
8. ~~**Activation code input for tier unlocking**~~ -- DONE. Commit `291ce22`. New TierManager utility manages effective tier (highest of BuildConfig.FLAVOR and activated_tier). Settings shows activation code input for Free tier. Server validates codes from activation_codes.json (TEST-PRO1-CODE, TEST-PREM-CODE for testing). On success, stores tier and restarts app. All feature gates (online status, anti-recording) use TierManager.
9. ~~**STEALTH-DELETE SOS function**~~ -- DONE. Commit `9334834`. Emergency data wipe via 5-tap trigger on "Reset App" in Settings. Wipes all SharedPreferences, files, databases, clipboard. Sends DEREGISTER to server. Restarts to fresh-install state with new SecureCall ID.
10. ~~**Inferno IFR Token integration**~~ -- DONE. Commit `9c1b83a`. Server uses ethers.js to call IFRLock.isLocked() on Ethereum (0x769928aB...). 1,000 IFR = Pro, 5,000 IFR = Premium. IfrLockManager handles wallet storage, verification, 24h re-check. Settings UI: wallet input, verify button, WalletConnect placeholder. Landing page: IFR Plan card with download button. Wiki: full ifr-unlock.html guide.
11. ~~**Anonymous eSIM + VPN integration**~~ -- DONE. Commit `615b6bc`. Premium-only: NetworkManager for eSIM network binding, GhostVpnService with WireGuard split tunnel (addAllowedApplication), kill switch, auto-reconnect. Settings UI: eSIM setup/routing, VPN enable/config/kill-switch. Free/Pro locked with "PREMIUM feature" label. Rollback: tag `v0.9-stable`.
12. **Firebase setup** -- Configure real Firebase credentials to enable FCM push for incoming calls when app is not running.
12. **TURN credential rotation** -- Move hardcoded Metered.ca TURN credentials out of `build.gradle` and fetch from server at runtime.
13. **Automated tests** -- Add unit/integration tests for the new features (crypto, jitter buffer, WebRTC signaling, phone lookup).
